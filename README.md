...
DX推進担当・情シス向けのPoCツール2本セット（Streamlitマルチページ構成）。
ファイル構成
ai-readiness-check/
├── app.py                   # トップページ（ツール選択）
├── pages/
...
└── README.md
セットアップ
powershellgit clone https://github.com/aaa473785-code/ai-readiness-check.git
...
※ 64bit Python 必須。Visual C++ ランタイムも必要。
起動
powershellstreamlit run app.py
ブラウザでトップページが開き、2つのツールを選択できる。
左のサイドバーからもページを切り替え可能。
① AI活用度セルフチェック
